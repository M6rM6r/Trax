...
interface WenshAppPercentageProps {
  app_percentage: string | number;
  type: string;
}

/**
 * Wensh service AppPercentage component
 * Handles percentage input with wensh type dropdown
 */
const WenshAppPercentage = ({
  app_percentage,
  type,
}: WenshAppPercentageProps) => {
  ...
  // Fetch wensh types enum
  const fetchSubType = async () => {
    try {
      const response = await fetcherClient<EnumDDLResponse>(
        `/enumDDLList?enum_name=WenshTypes`
      );
      setData(response.data);
    } catch (err) {
      console.error("Failed to fetch wensh types:", err);
    }
  };
...
export default WenshAppPercentage;
