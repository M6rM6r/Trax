...
export interface RuntimeEnvOverrides {
  ...
  NEXT_PUBLIC_DEFAULT_MAP_LAT?: string | number;
  NEXT_PUBLIC_DEFAULT_MAP_LNG?: string | number;
  NEXT_PUBLIC_DEFAULT_MAP_ZOOM?: string | number;
}
...
const envSchema = z
  .object({
    ...
    NEXT_PUBLIC_FIREBASE_APP_CHECK_KEY: z.string().optional(),
...
function loadEnv(): EnvConfig {
  const envSource = {
    ...
    NEXT_PUBLIC_FIREBASE_APP_CHECK_KEY: process.env.NEXT_PUBLIC_FIREBASE_APP_CHECK_KEY,
  ...
  try {
    ...
    if (!result.success) {
      ...
      return envSchema.parse({ NEXT_PUBLIC_USE_FIREBASE: "false" });
    }
    return result.data;
  } catch {
    return {
