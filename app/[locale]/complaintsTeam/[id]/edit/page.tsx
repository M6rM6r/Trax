...
const Page = ({ params }: { params: { id: string } }) => {
  const [data, setData] = useState<TeamResponse>({} as TeamResponse);
  ...
  useEffect(() => {
    const getTeam = async () => {
      try {
        const response = await fetcherClient<TeamResponse>(
          `/teams/${params.id}`
        );
        setData(response);
        setLoading(false);
    ...
    <MainLayout showSidebar={false}>
      ...
      {!loading && (
        <Formik<{
          ...
          initialValues={{
            title_ar: data.data.team?.title_ar,
            title_en: data.data.team?.title_en,
            is_active: data.data.team?.is_active?.toString(),
            categories: data.data.team.categories?.map((item: any) => item.id),
            manager: data.data.team.manager?.id,
            members: data.data.team.members?.map((item: any) => item.id),
          }}
          ...
          onSubmit={async (values, { setSubmitting }) => {
            ...
            try {
              const response = await fetcherClient<any>(`/teams/${params.id}`, {
                method: "POST",
                body: formdata,
              });
                ...
                <div className="flex flex-col gap-5 p-4 border border-gray200 rounded-6">
                  ...
                  <div className=" grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                    ...
                    <CustomSelect
                      ...
                      isMultiple
                      initialValue={props.values.categories}
                    />
                    <CustomSelect
                      name="manager"
                      ...
                      value="value"
                      className="md:col-span-2 lg:col-span-1"
                      initialValue={props.values.manager}
                    />
                    <CustomSelect
                      name="members"
                      title="أعضاء الفريق"
                      ...
                      isMultiple
                      initialValue={props.values.members}
                    />
                  </div>
                </div>
                {/* <DataTable
