...
import { clientsResponse } from "@/lib/types/responseTypes";
const Page = async ({
  searchParams,
}: {
  ...
  const data = await fetcher<clientsResponse>(
    `/getCustomersWithComplaintCounts?filters[type]=customer&${urlParams.toString()}`
  );
  // 
