...
const Page = ({ params }: { params: { id: string } }) => {
  const [data, setData] = useState<RuleResponse>({} as RuleResponse);
  ...
  useEffect(() => {
    const getRule = async () => {
      try {
        const response = await fetcherClient<RuleResponse>(
          `/rules/${params.id}`
        );
        setData(response);
        setLoading(false);
    ...
    <MainLayout showSidebar={false}>
      ...
      {!loading && (
        <Formik<{
          ...
          initialValues={{
            title_ar: data.data.rule.title_ar,
            title_en: data.data.rule.title_en,
            description_ar: data.data.rule.description_ar,
            description_en: data.data.rule.description_en,
            is_active: data.data.rule.is_active.toString(),
          }}
          ...
          onSubmit={async (values, { setSubmitting }) => {
            ...
            try {
              const response = await fetcherClient<any>(`/rules/${params.id}`, {
                method: "POST",
                body: formdata,
              });
