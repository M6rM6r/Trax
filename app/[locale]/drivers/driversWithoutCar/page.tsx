...
import { columns } from "@/components/Drivers/DriversTableColumns/SpecialColumns/DriverWithouCarColumns";
...
const Page = async ({
  ...
  const data = await fetcher<TaxiResponse>(
    `/drivers?filters[driver_without_car]=1&${params.toString()}`,
    { cache: "no-store" }
  );
  return (
    <MainLayout>
      ...
      <DataTable
        ...
        topComponent={
          <AddDriver vehicleType={EVehicleType.driver_without_car} />
        }
        exportLink="drivers"
