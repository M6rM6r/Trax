...
const Page = async ({
  ...
  const data = await fetcher<ServiceSettingsResponse>(
    `/towing/getServiceSettings?${searchParams.zone_id ? `zone_id=${searchParams.zone_id}` : ""
    }`,
    { cache: 'no-store' }
  );

  const peakTimesUrl = `/getPeakTimes?type=towing${searchParams.zone_id ? `&zone_id=${searchParams.zone_id}` : ""
  ...
  return (
    <MainLayout>
      ...
      <Prices
        ...
        distance_from_paved_road={getServiceSettingsValue(
          data.data,
        ...
        serviceName="towing"
      />
      <PeakPrices data={peakTimesData} serviceName="towing" />
      {searchParams.tab != "points" ? (
        <Positions serviceName="towing" />
      ) : (
        <PointsData />
      )}
    </MainLayout>
